...
public abstract class CommonSearchRepository<T extends ESDoc> {
	...
	protected final T findById(String id, Class<T> clazz) {
		return elasticsearchOperations.queryForObject(GetQuery.getById(id.toString()), clazz);
	}
	 ...
	 * @return
	 */
	protected final List<T> queryForList(CriteriaQuery query, Class<T> clazz, boolean allowAllIndices) {
