...
public abstract class CommonQueuePublisher {
	...
	protected final void publisher(String topicName, MessageWrapper value) {
		MessageProto messageProto = MessageProtoOuterClass.MessageProto.newBuilder()
				.setId((value.getId() != null ? value.getId() : 0l))
				.setStatus((value.getStatus() != null ? value.getStatus() : "")).setData(value.getByteString()).build();
	...
	protected final void publisher(String topicName, MessageWrapper value,
		...
		MessageProto messageProto = MessageProtoOuterClass.MessageProto.newBuilder()
				.setId((value.getId() != null ? value.getId() : 0l))
				.setStatus((value.getStatus() != null ? value.getStatus() : "")).setData(value.getByteString()).build();
		ListenableFuture<SendResult<String, byte[]>> future = kafkaTemplate.send(topicName, messageProto.toByteArray());
	...
	/**
	 * Overloaded publisher method to send with key's for partitioning and ACK on
	 * success and failure
	 * 
	 * @param topicName
	 * @param key
	...
	protected final void publisher(String topicName, String key, MessageWrapper value,
		...
		MessageProto messageProto = MessageProtoOuterClass.MessageProto.newBuilder()
				.setId((value.getId() != null ? value.getId() : 0l))
				.setStatus((value.getStatus() != null ? value.getStatus() : "")).setData(value.getByteString()).build();
		ListenableFuture<SendResult<String, byte[]>> future = kafkaTemplate.send(topicName, key,
				messageProto.toByteArray());
		future.addCallback(new ListenableFutureCallback<SendResult<String, byte[]>>() {
			@Override
			public void onSuccess(SendResult<String, byte[]> result) {
	...
	protected final MessageWrapper getMessageWrapper() {
		return MessageWrapper.newInstance().with($ -> {
