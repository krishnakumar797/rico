...
public class ElasticSearchConfig extends AbstractElasticsearchConfiguration {
	...
	public RestHighLevelClient elasticsearchClient() {
		...
		ClientConfiguration clientConfiguration = ClientConfiguration.builder().connectedTo(elasticServers)
				.withConnectTimeout(Duration.ofSeconds(5)).withSocketTimeout(Duration.ofSeconds(3))
				.withDefaultHeaders(defaultHeaders).withBasicAuth(elasticUser, elasticPassword).build();
