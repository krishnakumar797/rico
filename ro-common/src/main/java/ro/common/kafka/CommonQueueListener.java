...
public abstract class CommonQueueListener implements ConsumerSeekAware {
	 ...
	 */
	protected CommonQueueListener(Class<? extends CommonQueueListener> clas) {
		counter++;
		if (counter != 1) {
			throw new RuntimeException("CommonQueueListener is inherited more than once");
		}
	}
	...
	private void listener(ConsumerRecord<?, byte[]> consumerRecord) throws QueueException {
		try {
			...
			MessageWrapper wrapper = MessageWrapper.newInstance().with($ -> {
	...
	/**
	 * Method which determines the consumer seek position for a particular topic to
	 * the beginning.
	 */
	@Override
	public void onPartitionsAssigned(Map<TopicPartition, Long> assignments, ConsumerSeekCallback callback) {
		for (String topic : topicsToStartFromBeginning) {
			assignments.keySet().stream().filter(partition -> topic.equals(partition.topic())).forEach(partition -> {
				callback.seekToBeginning(topic, partition.partition());
			});
		}
	...
	/**
	 * Abstract method for data processing. Override this method inside the
	 * inherited class to handle the data received at kafka topics
	 * 
	 * @param topicName
	 * @param value
	 */
